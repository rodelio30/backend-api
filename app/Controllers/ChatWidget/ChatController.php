...
class ChatController extends GeneralCW
{
    ...
    private function getAuthenticatedClientId(): ?int
    {
        ...
        if (!$tokenObject || empty($tokenObject->data->id)) {
            return null;
        }

        return (int) $tokenObject->data->id;
    }
    // Get Chat Sessions (API)
    public function apiGetChatSessions()
    {
    ...
    // Close Chat Session (API)
    public function apiCloseSession()
    {
        $clientId = $this->getAuthenticatedClientId();
        $data = $this->request->getJSON(true);
    ...
    // Get Agent Workload (API)
    public function apiGetAgentWorkload()
    {
        $clientId = $this->getAuthenticatedClientId();
        ...
        // $agents = $this->userCWModel
        $agents = $this->agentModel
            ->select('id, username, current_chats, max_concurrent_chats, status, is_online')
            ->where('client_id', $clientId)
            ->where('status', 'active')
            // ->where('client_id', $clientId)
            // ->whereIn('role', ['admin', 'support'])
    ...
    /**
     * Guard for public testing API
     */
    private function allowPublicChatApi(): bool
    {
        // ENV flag
        if (getenv('ENABLE_PUBLIC_CHAT_API') !== 'true') {
            return false;
        }

        // Optional simple key
        $publicKey = $this->request->getHeaderLine('X-PUBLIC-KEY');
        if ($publicKey && $publicKey !== 'chat-test-123') {
            return false;
        }
    ...
    public function publicStartSession()
    {
        if (!$this->allowPublicChatApi()) {
            return $this->response->setStatusCode(403)->setJSON([
                'status' => 'error',
                'message' => 'Public Chat API disabled'
            ]);
        }

        $data = $this->request->getJSON(true);

        // $customerName = $this->sanitizeInput($data['customer_name'] ?? 'Anonymous'); // Not working as expected
    ...
    public function publicSendMessage()
    {
        ...
        $sessionId = $this->sanitizeInput($data['session_id']);
        ...
        $mongo = new \App\Models\MongoMessageCWModel();

        $mongo->insertMessage([
            'session_id' => $sessionId,
            'sender_type' => 'customer',
            'sender_name' => 'Public Tester',
            'message' => $message,
            'message_type' => 'text',
            'created_at' => date('Y-m-d H:i:s')
        ]);
    ...
    // Get Messages (Public)
    public function publicGetMessages($sessionId)
    {
        if (!$this->allowPublicChatApi()) {
            return $this->response->setStatusCode(403)->setJSON([
                'status' => 'error',
        ...
        $mongo = new \App\Models\MongoMessageCWModel();
        $messages = $mongo->getSessionMessages($sessionId);
    ...
    // Close Session (Public)
    public function publicCloseSession()
    {
        if (!$this->allowPublicChatApi()) {
            return $this->response->setStatusCode(403)->setJSON([
                'status' => 'error',
