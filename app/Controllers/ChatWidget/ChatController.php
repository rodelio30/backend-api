...
use App\Models\ClientWidgetSettingModel;
use Exception;

class ChatController extends GeneralCW
{
    protected ApiKeyModel $apiKeyModel;
    protected ClientWidgetSettingModel $clientWidgetSettingModel;
    ...
    public function __construct()
    {
        ...
        $this->clientWidgetSettingModel = new ClientWidgetSettingModel();
    }
    ...
    public function index()
    {
        ...
        if ($isIframe && $apiKey) {
            $apiKeyModel = new \App\Models\ApiKeyCWModel();
        ...
        if ($apiKey) {
            try {
                $apiKeyRecord = $this->apiKeyModel
                    ->where('api_key', $apiKey)
                    ->where('status', 'active')
                    ->first();

                if ($apiKeyRecord) {
                    $settings = $this->clientWidgetSettingModel->getByClientId((int) $apiKeyRecord['client_id']);
                    if (!empty($settings)) {
    ...
    public function startSession()
    {
        ...
        if ($clientId === null) {
            ...
            $apiKeyModel = $apiKeyModel ?? new \App\Models\ApiKeyCWModel();
